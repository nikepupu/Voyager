...
def rules(env, notes=True):
    prompt = """
        You are playing minecraft with humans by controlling bots. \n 
        You are playing a collaborative cooking game. \n
        The goal is to cook as many dishes as possible. \n """
    # prompt = 'You are playing minecraft with other bots. \n'

    prompt += 'The available actions are :\n'
    prompt += '1) goto: goto a tool location \n'
    prompt += '2) mineBlock: mine a block \n'
    prompt += '3) killMob: kill the mob entity \n'
    prompt += '4) putFuelFurnace: put potential fuels into the furnace \n'
    prompt += '5) takeOutFurnace: take out the cooked ingredients from the tool \n'
